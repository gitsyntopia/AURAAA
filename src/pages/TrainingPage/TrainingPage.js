...
export default function TrainingPage({ onEndSession, onHome, streamRef }) {
    ...
    pose.onResults((res) => {
      if (res.poseLandmarks) {
        ...
        if (L && R && nose) {
          ...
          const headTilt = Math.atan2(R.y - L.y, R.x - L.x) * (180 / Math.PI);
          const eyeContact = Math.abs(nose.x - 0.5) < 0.05 ? "Good" : "Looking Away";
          setMetrics(prev => ({ ...prev, posture: Number(postureScore.toFixed(0)), headTilt: Number(headTilt.toFixed(0)), eyeContact }));
        }
      }
    });
  ...
  const startAudioStreaming = async (sid=null) => {
    try {
      ...
      ws.onclose = () => console.log("🔴 WebSocket audio closed");
      ws.onerror = (e) => console.error("WS audio error:", e);
      ...
      console.error("startAudioStreaming error:", err);
      setError("Microphone access failed or WebSocket connection error.");
    }
  };
